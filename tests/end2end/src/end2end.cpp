...
void generator::generate_optimal_orders(size_type test_number) {
    ...
    auto tmp = random_value(MIN_MATRICES_NUMBER, MAX_MATRICES_NUMBER);
    for (size_type count = 0; count < matrices_number; ++count) {
        matrices_sizes_.push_back(tmp);
        matrices_sizes_.push_back(tmp = random_value(MIN_MATRICES_NUMBER, MAX_MATRICES_NUMBER));
    }

    effective_order_.reserve(matrices_number - 1);
    auto size = matrices_sizes_.size();
    yLAB::Matrix<size_type> lookup_table {size + 1, size + 1, 0};
    fill_optimal_order(0, size - 1, lookup_table);

    write_data_into_files(test_number);
}
...
void generator::write_data_into_files(size_type test_number) {
    ...
    for (auto val : matrices_sizes_) {
        test_file << val << std::endl;
    }

    for (auto val : effective_order_) {
        ans_file << val << std::endl;
    }

    effective_order_.clear();
